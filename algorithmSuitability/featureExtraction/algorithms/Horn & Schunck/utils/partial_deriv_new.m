...
%         h         = [-1 0 1]/2;
        h = [-1 1]; %[-1 9 -45 0 45 -9 1]/60;        % derivative used by Bruhn et al "combing "IJCV05' page218
    end;
    ...
    if nargout == 3
        ...
        if size(images, 4) == 1
            % gray-level
            Ix      = imfilter(images(:,:,2), h,  'corr', 'symmetric', 'same');  
            Iy      = imfilter(images(:,:,2), h', 'corr', 'symmetric', 'same');
            x3      = floor(x2);
            y3      = floor(y2);            
            
            Ix      = interp2(x,y,Ix,x3,y2,'linear', NaN);
            Iy      = interp2(x,y,Iy,x2,y3,'linear', NaN);
