classdef FlowAngleVarianceFeature < AbstractFeature
    %FLOWANGLEVARIANCEFEATURE computes the E[\atan(u,v)^2] where u and v
    %   are the vectors for all the flow algorithms i.e. this feature 
    %   computes the variance in flow angle across all the candidate flow 
    ...
    properties (Constant)
        FEATURE_TYPE = 'Flow Angle Variance';
        FEATURE_SHORT_TYPE = 'FA';
    end
    
    
    methods
        function obj = FlowAngleVarianceFeature( cell_flows, varargin )
        ...
        %   with a depth equivalent to the number of scales
            ...
            if obj.no_scales > 1
                ...
                flowangvar = zeros(calc_feature_vec.image_sz(1), calc_feature_vec.image_sz(2), obj.no_scales);
                ...
                for scale_idx = 1:obj.no_scales
                    ...
                    % angle variance
                    ang = squeeze(atan(uv_resized(:,:,2,:) ./ uv_resized(:,:,1,:)));
                    ang(isnan(ang)) = 0;
                    avg_ang = anglesUnwrappedMean( ang, 'rad', 3 );
                    avg_ang = repmat(avg_ang, [1 1 size(ang,3)]);
                    avg_ang = anglesUnwrappedDiff(ang, avg_ang);
                    
                    avg_ang = mean(avg_ang.^2, 3);
                    
                    % resize it to the original image size
                    flowangvar(:,:,scale_idx) = imresize(avg_ang, calc_feature_vec.image_sz);
                end
            else
                ...
                % angle variance
                ang = squeeze(atan(uv_flow(:,:,2,:) ./ uv_flow(:,:,1,:)));
                ang(isnan(ang)) = 0;
                avg_ang = anglesUnwrappedMean( ang, 'rad', 3 );
                avg_ang = repmat(avg_ang, [1 1 size(ang,3)]);
                avg_ang = anglesUnwrappedDiff(ang, avg_ang);
