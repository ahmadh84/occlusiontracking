classdef ReverseFlowAreaFeature < AbstractFeature
    ...
    properties (Constant)
        NAN_VAL = 0;
        FEATURE_TYPE = 'Reverse Flow Area * Photo Constancy';
        FEATURE_SHORT_TYPE = 'RSmPC';
    end
    
    
    methods
        function obj = ReverseFlowAreaFeature( cell_flows, varargin )
        ...
        function [ revflowarea feature_depth compute_time ] = calcFeatures( obj, calc_feature_vec )
        ...
        %   number of scales
            ...
            if obj.no_scales > 1
                ...
                revflowarea = zeros(calc_feature_vec.image_sz(1), calc_feature_vec.image_sz(2), no_flow_algos*obj.no_scales);
                ...
                for scale_idx = 1:obj.no_scales
                    ...
                    for algo_idx = 1:no_flow_algos
                        ...
                        area = 0.5 .* abs((-uv_resized(:,:,1) .* rev_v(ind_dash)) + (uv_resized(:,:,2) .* rev_u(ind_dash)));
                        area = area .* (proj_im.^2);
                        
                        area(outside_idcs) = ReverseFlowAreaFeature.NAN_VAL;
                        
                        % store
                        revflowarea(:,:,((algo_idx-1)*obj.no_scales)+scale_idx) = imresize(area, calc_feature_vec.image_sz);
                    end
                end
            else
                ...
                % initialize the output feature
                revflowarea = zeros(calc_feature_vec.image_sz(1), calc_feature_vec.image_sz(2), no_flow_algos);
                ...
                for algo_idx = 1:no_flow_algos
                    ...
                    reverse_dist(outside_idcs) = ReverseFlowAreaFeature.NAN_VAL;
                    
                    % store
                    revflowarea(:,:,algo_idx) = reverse_dist;
                end
            end
            
            feature_depth = size(revflowarea,3);
