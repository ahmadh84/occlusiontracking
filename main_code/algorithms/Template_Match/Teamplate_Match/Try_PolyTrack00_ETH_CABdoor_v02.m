...
I_Cur = im2double(imread( 'C:\users\brostow\data\CABside\cam3_Small\Cam3p2_001192.png' ));
I_Prev = im2double(imread( 'C:\users\brostow\data\CABside\cam3_Small\Cam3p2_001194.png' ));
...
SearchPadding = 0.75 * [20 20 20 20];
...
numPolys = size(Polys,1);
% WhatChannels = [2]; % So just green.
% Iref = I_Cur;
% I = I_Prev;
[H, S, V]   = rgb2hsv( I_Cur(:,:,1), I_Cur(:,:,2), I_Cur(:,:,3) );
Iref        = cat(3, H, V );
[H, S, V]   = rgb2hsv( I_Prev(:,:,1), I_Prev(:,:,2), I_Prev(:,:,3) );
I           = cat(3, H, V );
WhatChannels = [1 2]; % Hue and Value
for( iPoly = 1:numPolys )
    % First look how the Left poly moved:
    display( iPoly );
    [ movedSE scoreMaxs ] = PolyPatchMatchesWhere(Iref, I, Polys{iPoly,1}, SearchPadding, numKeepers, WhatChannels);
    WholeImgPolys_MovedSE{iPoly, 1} =   movedSE;
    WholeImgPolys_ScoresMax{iPoly, 1} = scoreMaxs;

    % Now check the Right poly's motion:
    [ movedSE scoreMaxs ] = PolyPatchMatchesWhere(Iref, I, Polys{iPoly,2}, SearchPadding, numKeepers, WhatChannels);
...
%for( iPoly = 1:numPolys ) scoresL(iPoly) = WholeImgPolys_ScoresMax{iPoly, 1}(1); end
    ...
    for( iPoly = 1:numPolys )
        ...
        threshMinScoreToBeStatic = 3.0; %3.0;
...
%                 color(2) = 0.5;    continue;
%             elseif( matchQual < (3.0-threshMinScoreToBeStatic) )
%                 continue;
%             else
%                 color(3) = 0.9;    continue;
%             end
