function y = gaussian(x, sigma, type)
%GAUSSIAN   Gaussian "robust" function.
%   GAUSSIAN(X, SIGMA, TYPE) evaluates the Gaussian "robust" function
...
% OR IN CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE.
  ...
  switch (type)
   case 0
    y = 0.5 * log(2 * pi) + log(sigma) + 0.5 * x.^2 / sigma^2;
   case 1
    y = x / sigma^2;
   case 2
    y = repmat(1 / sigma^2, size(x));
  end