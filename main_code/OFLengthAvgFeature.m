classdef OFLengthAvgFeature < AbstractFeature
    %OFLENGTHAVGFEATURE computes the E[(||u,v||)^2] where u and v are the 
    %   vectors for all the flow algorithms i.e. this feature computes the 
    %   variance in flow lengths across all the candidate flow algorithms 
    %   being used. The constructor takes a cell array of Flow objects 
    %   which will be used for computing this feature. The user is required 
    %   to provide 2 or more candidate flow algorithms. The constructor 
    %   also optionally takes a size 2 vector for computing the feature on 
    ...
    %   to calcFeatures should have extra_info.flow_scalespace (the 
    %   scalespace structure), apart from image_sz. Note that it is the 
    %   responsibility of the user to provide enough number of scales in 
    %   all 3 scalespace structures. If using scalespace, 
    %   extra_info.uv_flows is required for computing this feature. If 
    %   using the scalespace, usually, the output features go up in the 
    %   scalespace (increasing gaussian std-dev) with increasing depth.
    %
    ...
    properties (Constant)
        FEATURE_TYPE = 'Flow Length Average';
        FEATURE_SHORT_TYPE = 'FLA';
    end
    
    
    methods
        function obj = OFLengthAvgFeature( cell_flows, varargin )
        ...
        function [ flowlenavg feature_depth compute_time ] = calcFeatures( obj, calc_feature_vec )
        ...
        %   with a depth equivalent to the number of scales
            ...
            if obj.no_scales > 1
                ...
                flowlenavg = zeros(calc_feature_vec.image_sz(1), calc_feature_vec.image_sz(2), no_flow_algos*obj.no_scales);
                ...
                for scale_idx = 1:obj.no_scales
                    ...
                    for algo_idx = 1:no_flow_algos
                        len(:,:,algo_idx) = filter2(ones(5)./25, len(:,:,algo_idx), 'same');
                    
                        % resize it to the original image size
                        flowlenavg(:,:,((algo_idx-1)*obj.no_scales)+scale_idx) = imresize(len(:,:,algo_idx), calc_feature_vec.image_sz);
                    end
                end
            else
                ...
                % length variance
                len = squeeze(sqrt(uv_flow(:,:,1,:).^2 + uv_flow(:,:,2,:).^2));
            ...
            feature_depth = size(flowlenavg,3);
