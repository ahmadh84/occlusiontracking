function [phi,frames,numIter,speed_grad] = evolve_height_function_N(img, occl_mask, time_step, num_iterations, speed_type, display_interval, boundary_speed_interval, numSuperPixels)
    ...
    if (nargin < 7 || isempty(boundary_speed_interval))
        boundary_speed_interval = 6;
    end
    if (nargin < 8 || isempty(numSuperPixels))
        numSuperPixels = 200;
    end
    ...
    if (strcmp(speed_type,'superpixels'))
        ...
        [speed_grad,speed_grad_x,speed_grad_y] = get_speed_based_on_gradient(smooth_img, occl_mask, [], normSigma, phi);
        speed = speed_grad;
        ...
        phi = DT(input_dist);       % compute the distance transform psi
    ...
    for i = 1:num_iterations
        ...
        if (strcmp(speed_type,'superpixels'))
            if (i < 20)
                ...
                speed = get_speed_based_on_gradient(smooth_img, occl_mask, 1, normSigma, phi, speed_grad_extended, speed_grad_x_extended, speed_grad_y_extended);
            end
        end
