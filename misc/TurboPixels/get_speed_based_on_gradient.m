function [speed,speed_x,speed_y,doublet,curvature] = get_speed_based_on_gradient(img, occl_mask, doDoublet, normSigma, phi, speed, speed_x, speed_y)
    
    % Flag controlling relative vs. absolute gradient magnitude
    if (nargin < 3 || isempty(doDoublet))
        doDoublet = 0;
    end
    
    if (nargin < 4 || isempty(normSigma))
        normSigma = 5;
    end

    
    if (nargin < 6 || isempty(speed))
        if isempty(occl_mask)
            [gx,gy] = height_function_der(255*img);
        else
            [gx,gy] = height_function_der(128*img);

            % EDITS Begin
            [gx2,gy2] = height_function_der(127*occl_mask);
            gx = gx2 + gx;
            gy = gy2 + gy;
            % EDITS End
        end
